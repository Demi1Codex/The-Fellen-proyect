...
class PlayState extends MusicBeatState
{
	...
	@:dox(hide) override public function create()
	{
		...
		var leftColor:Int = dad != null && dad.iconColor != null && Options.colorHealthBar ? dad.iconColor : (opponentMode ? 0xFF1500FF : 0xFF9900FF);
		var rightColor:Int = boyfriend != null && boyfriend.iconColor != null && Options.colorHealthBar ? boyfriend.iconColor : (opponentMode ? 0xFF9900FF : 0xFF1500FF); // switch the colors
	...
	function updateRatingStuff() {
		...
		missesTxt.text = '${comboBreaks ? "Combo Breaks" : "Weoneses"}:$misses';
		...
		@:privateAccess {
			...
			accuracyTxt.text = 'Rangos:${accuracy < 0 ? "-%" : '${CoolUtil.quantize(accuracy * 100, 100)}%'} - ${curRating.rating}';
